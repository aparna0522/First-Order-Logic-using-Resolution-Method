...
public:
    Tree(vector<string> &val, int start, int end)
    {
        ...
        // Find the highest precedence operator from the given val vector, save in splitIndex
        for (int i = start; i < end; i++)
        {
            ...
            else if (val[splitIndex] != "|" && val[splitIndex] != "=>" && val[splitIndex] != "&")
            {
        ...
        For A|B=>C
         ...
         A     B
        */
        if (val[splitIndex] != "|" && val[splitIndex] != "=>" && val[splitIndex] != "&")
        {
    ...
    vector<Tree *> DistributiveProp()
    {
        ...
        */
        if (leftChild)
            left = leftChild->DistributiveProp();
        if (rightChild)
            right = rightChild->DistributiveProp();
    ...
    vector<string> finalAnswer()
    {
        ...
        vector<string> right;
        if (leftChild)
            left = leftChild->finalAnswer();
        if (rightChild)
            right = rightChild->finalAnswer();
...
bool isConstant(vector<string> &sentence)
{
    for (auto &predicate : sentence)
    {
        for (int i = 0; i < predicate.size(); i++)
        {
            ...
            {
                if (predicate[i + 1] >= 'a' && predicate[i + 1] <= 'z')
                    return false;
            }
        }
    }
...
bool unify(string &A, string &B, map<string, string> &theta)
{
    ...
    for (int i = 0; i < paramsA.size(); i++)
    {
        if (checkIfVariable(paramsA[i]))
            theta[paramsA[i]] = ufo.Find(paramsA[i]);
        if (checkIfVariable(paramsB[i]))
                ...
                if (areComplements(sentence[i], sentence[j]))
                {
                    ...
                    if (unify(sentence[i], sentence[j], theta))
                    {
                        ...
                        for (auto &entry : theta)
                        {
                            ...
                            string complement = var[var.size() - 1] == '2' ? var.substr(0, var.size() - 1) : var + "2";
                            if (theta.find(complement) != theta.end() && theta[complement] != entry.second)
                            {
                                consistent = false;
                                break;
                            }
                        ...
                        for (int k = 0; k < sentence.size(); k++)
                        {
                            ...
                            string finalNegated = finalString[0] == '~' ? finalString.substr(1, finalString.size() - 1) : "~" + finalString;
...
vector<string> substitute(vector<string> &sentence, vector<string> &queries, map<string, string> &theta)
{
    vector<string> A;
    for (int i = 0; i < sentence.size(); i++)
    {
    ...
    while (resolved)
    {
        ...
        for (int i = 0; i < KB.size(); i++)
        {
            if (KB[i].size() == 1 && isConstant(KB[i]))
            {
                ...
                {
                    if (areComplements(KB[i][0], sentence[j]) && areConstantComplements(KB[i][0], sentence[j]))
                    {
                        set<string> temp;
                        for (auto &x : KB[i])
                        {
...
void DisplayQuery(vector<string> &sentence, vector<string> &query, vector<string> &resultant)
{
    for (int i = 0; i < sentence.size(); i++)
    {
        cout << sentence[i];
...
bool subsumes(string &A, string &B)
{
    ...
    if (A_name != B_name || A_param.size() != B_param.size())
        return false;
...
bool subsumption(vector<string> &query, vector<vector<string>> &KB)
{
    for (auto &sentence : KB)
    {
        ...
        if (subsumptions >= sentence.size())
            return true;
    }
    return false;
}

bool resolution(vector<vector<string>> &KB, vector<string> &queries, int closed)
{
        ...
        for (int i = 0; i < KB.size(); i++)
        {
            ...
            for (auto &literal : sentence)
            {
                if (areComplements(literal, query))
                {
                    ...
                    else if (!checkIn(resolutionAnswer, KB)) // Check if resolution is not in KB instead
                    {
...
int main()
{
    ...
    else
    {
        ...
        queryAsked = queryAsked[0] == '~' ? queryAsked.substr(1, queryAsked.size() - 1) : "~" + queryAsked;
        ...
        // {
        //     cout << predA << " & " << predB << " Unification possible, mapping is\n";
        //     for (auto &x : theta)
